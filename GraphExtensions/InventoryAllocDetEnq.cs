...
{
	public class InventoryAllocDetEnqExtPlan6C : PXGraphExtension<InventoryAllocDetEnq>
    {
        ...
        */
        public virtual void AggregateCustomType(InventoryAllocDetEnqFilter aDest, InventoryAllocDetEnqResult aSrc)
        {
            ...
            if (aSrc.AllocationType == nameof(INAvailabilitySchemeExtPlan6C.usrInclQtySOTransfer))
            {
                InventoryAllocDetEnqFilterExtPlan6C aDestext = aDest.GetExtension<InventoryAllocDetEnqFilterExtPlan6C>();
        ...
        public virtual IEnumerable filter()
        {
            ...
            foreach (InventoryAllocDetEnqFilter filter1 in Base.Filter.Select())
            {
                ...
                InventoryAllocDetEnqFilterExtPlan6C filterext = filter1.GetExtension<InventoryAllocDetEnqFilterExtPlan6C>();
                ...
                if (filter1 != null && filter1.InventoryID != null)
                {
                    ...
                    foreach (InventoryAllocDetEnqResult it in
                      ...
                      Where(f => f.AllocationType == nameof(INAvailabilitySchemeExtPlan6C.usrInclQtySOTransfer))) //???
                    {
                        AggregateCustomType(filter1, it);
                    }


                    INAvailabilitySchemeExtPlan6C availSchemeRecext = availSchemeRec.GetExtension<INAvailabilitySchemeExtPlan6C>();
          ...
          InventoryAllocDetEnq.ItemPlanWithExtraInfo itemPlanWithExtraInfo)
        {
              ...
              && itemPlanWithExtraInfo.ItemPlan.OrigPlanType == InPlanConstantsSOTransfers.Plan6C)
            {
                foreach (var soOrderLink in itemPlanWithExtraInfo.ShipmentItemPlanToSOOrderItemPlanLinks)
                {
                    ...
                    {
                        Base.AdjustPrevResult(typeof(INPlanTypeExtPlan6C.usrInclQtySOTransfer), resultList, itemPlanWithExtraInfo.ItemPlan,
